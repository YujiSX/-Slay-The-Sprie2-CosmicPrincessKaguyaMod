...
{
    public sealed class KongoPhaseTransitionPower : CustomPowerModel
    {
        ...
        {
            if (!wasRemovalPrevented && creature == Owner && Owner.Monster is TsukitoKongo kongo)
            {
                _isReviving = true;
                await kongo.TriggerDeadState();  // 对应实验体的 TriggerDeadState
            }
        }
